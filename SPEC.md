...
- `lead_id`
- `stage_trace`: what each of Validate, Sanitize, and Dedup found on this row (validation errors, sanitization changes made, dedup match found and against which lead_id). These stages annotate the row; none of them may skip it past the Judge.
...
- The message contains sensitive content (a PII request, legal or compliance-flavored language, or similar; exact definition still open from the first conversation) that shouldn't be auto-decided.
...
Dedup flags a row as linked to an earlier `lead_id`. Once flagged, the row still goes through the full evaluation above on its own content, same Pipeline rule as every other row, no special-casing that skips the Judge or the criteria.

- **Consistent with the earlier lead** (matching identity signals, and budget/ask that's the same or doesn't contradict the earlier submission): decide the row on its own merits, exactly like any other row. In practice this usually reproduces the earlier `lead_id`'s decision, since the content is the same or compatible, that's a consequence of evaluating it honestly, not a special "duplicates get decision X" rule. The `reason` must state the duplicate relationship and the earlier `lead_id` explicitly, so it's visible in decisions.csv itself, not just in run_log.json's `stage_trace`.
- **Conflicts with the earlier lead** (different budget, different company, a materially different ask, anything that contradicts rather than confirms): ESCALATE. This is the cross-row version of the field-conflict bullet above: two submissions claiming to be the same lead but disagreeing with each other is exactly the kind of thing that can't be resolved from the rows alone.

Not resolved here, flagged open with the same provisional status as Section 6's: how confident Dedup's own matching needs to be before treating two rows as linked at all. This assumes a Dedup match is trustworthy once made; the matching logic itself (exact-field match vs. fuzzy similarity) is an M2 question, not decided in this taxonomy.
